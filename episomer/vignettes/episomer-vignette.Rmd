...
`episomer` uses the Bluesky search API 'searhPost' endpoint [*https://bsky.social/xrpc/app.bsky.feed.searchPosts*]. This service enable authenticated users to access published messages free of charge. There are no mentions about exhaustiviness on the documentaton but it appears to return same results as Bluesky search feature in the user app. There is no limit on the number period of messaged that can be obtained but by defaulr, episomer will limit to last 30 days (this parameter can be changed in the configuration page)  HERE It searches against a sample of recent posts published in the past 7 days and it focuses on relevance and not completeness. This means that some posts and users may be missing from search results.
