...
json2lucene <- function(tasks = get_tasks(), chunk_size = 400) {
  ...
  if (!file.exists(get_search_archive_path()))
    dir.create(get_search_archive_path(), recursive = TRUE)
  ...
  dates <- sort(unique(unlist(c(
    get_file_names(
      get_search_path(),
      pattern = "[0-9][0-9][0-9][0-9]\\.[0-9][0-9]\\.[0-9][0-9].*\\.json\\.gz$"
    ),
    get_file_names(
      get_geo_path(),
      pattern = "[0-9][0-9][0-9][0-9]\\.[0-9][0-9]\\.[0-9][0-9].*\\.json\\.gz$"
  ...
  for (i in 1:length(dates)) {
    ...
    tasks <- update_dep_task(
      tasks,
      "running",
      paste("Scanning files for ", dates[[i]])
    )
    files <- c(
      lapply(
        search_files,
    ...
    if (length(files) > 0) {
      tasks <- update_dep_task(
        tasks,
        "running",
        paste("Iterating over ", length(files), "files ")
      )
      for (j in 1:length(files)) {
        ...
        lines <- readLines(
          con = con,
          n = if (files[[j]]$type == "geo") 100 * chunk_size else chunk_size
        )
        file_lines <- 0
        ...
        while (length(lines) > 0) {
          ...
          time.taken <- as.numeric(difftime(
            Sys.time(),
            start.time,
            units = 'secs'
          ))
          tasks <- update_dep_task(
            ...
            paste0(
              "\r",
              round(100 * estimate_read_size / total_size, 2),
              "%. ",
              round(estimate_read_size / (1024 * 1024)),
              "/",
              round(total_size / (1024 * 1024)),
              "MB",
              " ",
              round(read_size / (1024 * 1024 * time.taken), 2),
              "MB/sec",
              " remaining time ",
              sec2hms(
                (time.taken * (total_size - estimate_read_size)) /
                  estimate_read_size
              ),
              " req/read/index: ",
              length(lines),
              ...
              dates[[i]],
              " ",
              if (files[[j]]$type == "search") files[[j]]$topic else
                "geolocation",
              "' "
            )
          )
      ...
      tasks <- update_dep_task(
        tasks,
        "running",
        paste("Archiving files for ", dates[[i]])
      )
...
archive_search_geo_file <- function(file) {
  ...
  froms <- c(
    x,
    file.path(dirname(x), paste0(".", basename(x), ".crc")),
    file.path(dirname(x), paste0("._SUCCESS.crc")),
    file.path(dirname(x), paste0("._SUCCESS.crc"))
  )
  # renaming if destination files exists already
  for (from in froms) {
    if (file$type == "search")
      to <- sub("search", 'search_archive', from) else if (file$type == "geo")
      to <- sub("geolocated", 'geo_archive', from) else
      stop(
        "The provided file to archive must be a named list with properties type (geo or search) and path"
      )
    todir <- dirname(to)
    ...
    if (file.exists(to)) {
      to <- gsub(
        ".json.gz",
        paste0(floor(stats::runif(1, min = 1, max = 500)), ".json.gz"),
        to
      )
    }
  ...
  while (
    grepl(paste0('^', get_geo_path()), fromdir) ||
      grepl(paste0('^', get_search_path()), fromdir)
  ) {
    if (
      dir.exists(fromdir) && length(list.files(fromdir, include.dirs = T)) == 0
    ) {
...
store_geo <- function(lines, created_dates, async = T, chunk_size = 100) {
  ...
  if (length(bad_lines) > 0)
    message(paste(
      ...
      "of",
      length(lines),
      "bad line(s) found!! These will be ignored"
    ))
  ...
  chunks <- lapply(
    1:ngroupes,
    function(g)
      lines[sapply(1:length(lines), function(ix) (ix %% (ngroupes)) == (g - 1))]
  )
  chunks <- lapply(
    chunks,
    function(chunk) paste0("[", paste0(chunk, collapse = ", "), "]")
  )
  if (length(chunks) > 0) {
    if (async) {
      ...
      if (length(status[status == 406]) > 0)
        message(paste(
          "Storage rejected ",
          length(status[status == 406]),
          "geolocation request for not having a valid content"
        ))
...
store_v1_search <- function(lines, topic, async = T) {
  ...
  if (length(lines) > 0) {
    if (async) {
      ...
      dates <- unique(unlist(
        lapply(
          async$parse()[status == 200],
          function(l) jsonlite::parse_json(l)$dates
        ),
        recursive = T
      ))
      return(dates)
