...
register_fs_runner <- function() {
  stop_if_no_config(paste(
    "Cannot check running status for fs without configuration setup"
  ))
  register_runner("fs")
}
...
register_fs_monitor <- function() {
  stop_if_no_config(paste(
    "Cannot check running status for fs monitor without configuration setup"
  ))
...
register_runner_task <- function(task_name) {
  ...
  stop_if_no_config(paste(
    "Cannot register scheduled task without configuration setup"
  ))
  ...
  if (.Platform$OS.type == "windows") {
    ...
    if (requireNamespace("taskscheduleR", quietly = TRUE)) {
      ...
      taskscheduleR::taskscheduler_create(
        ...
        schedule = "HOUR",
        startdate = if (conf$force_date_format == "")
          tail(strsplit(shell("echo %DATE%", intern = TRUE), " ")[[1]], 1) else
      ...
      run_attached <- TRUE
      warning(
        "Please install taskscheduleR package in order to permanently activate the task. This will run the task attached to this process and will end when you close this session."
      )
    }
  } else {
    run_attached <- TRUE
    warning(
      "Task scheduling is not implemented yet on this OS. You can still schedule it manually. Please refer to package vignette. Running the task attached to this process, it will end when you close this session"
    )
  }
...
kill_task <- function(pidfile, type = "R") {
  ...
  if (file.exists(path)) {
    ...
    if (.Platform$OS.type == "windows") {
      if (type == "R")
        image = "R\\.exe|Rscript\\.exe|rsession\\.exe|Rterm\\.exe" else if (
        type == "java"
      )
        image = "java.exe" else stop("Unsupported type of process")
      if (
        length(grep(
          image,
          system(
            paste('tasklist /nh /fi "pid eq ', last_pid, '"'),
            intern = TRUE
          )
        )) >
          0
      ...
      if (type == "R") image = "R\\|rsession" else if (type == "java")
        image = "java" else stop("Unsupported type of process")
      if (
        system(
          paste(
            "ps -cax | grep ",
...
missing_search_jobs <- function() {
  ...
  not_running <- active[sapply(
    active,
    function(sm) get_running_task_pid(sprintf("search.%s", sm)) < 0
  )]
  not_running
}
...
is_fs_running <- function() {
  ...
  tryCatch(
    httr::GET(url = get_scala_ping_url(), httr::timeout(1.0))$status_code ==
      200,
...
is_detect_running <- function() {
  stop_if_no_config(paste(
    "Cannot check detect batch status for search without configuration setup"
  ))
...
get_running_task_pid <- function(name) {
  ...
  if (file.exists(pid_path)) {
    ...
    pid_running <- (if (.Platform$OS.type == "windows") {
      length(grep(
        "R\\.exe|Rscript\\.exe|rsession\\.exe|Rterm\\.exe",
        system(paste('tasklist /nh /fi "pid eq ', last_pid, '"'), intern = TRUE)
      )) >
        0
    } else if (.Platform$OS.type == "mac")
      system(
        paste("ps -cax | grep 'R\\|rsession' | grep ", last_pid),
        ignore.stdout = TRUE
      ) ==
        0 else
      system(
        ...
        0)
    if (pid_running) last_pid else -1
  } else -1
}
...
get_tasks <- function(statuses = list()) {
  ...
  tasks <- if (file.exists(tasks_path)) {
    ...
    for (i in 1:length(t)) {
      t[[i]]$scheduled_for <- if (inherits(t[[i]]$scheduled_for, "numeric"))
        as.POSIXlt(t[[i]]$scheduled_for / 1000, origin = "1970-01-01") else NA
      t[[i]]$started_on <- if (inherits(t[[i]]$started_on, "numeric"))
        as.POSIXlt(t[[i]]$started_on / 1000, origin = "1970-01-01") else NA
      t[[i]]$end_on <- if (inherits(t[[i]]$end_on, "numeric"))
        as.POSIXlt(t[[i]]$end_on / 1000, origin = "1970-01-01") else NA
      t[[i]]$status <- if (inherits(t[[i]]$status, "character"))
        t[[i]]$status else NA
    }
    t
  } else {
  ...
  } else if (exists("alerts", where = tasks) && tasks$alerts$order != 3) {
    #dealing with bad order case after migration
  ...
  update_times <- setNames(
    ...
    sapply(conf$languages, function(l) l$code)
  ) # time where the change on languages was required by the shiny app
    ...
    function(c)
      (c %in% task_codes) &&
        ...
        (is.na(tasks$languages$started_on) ||
          ...
          (!is.null(update_times[[c]]) &&
            strptime(update_times[[c]], "%Y-%m-%d %H:%M:%S") >
              tasks$languages$started_on) ||
  ...
  tasks$languages$vectors = lapply(tasks$languages$codes, function(c) {
    conf$tasks$languages = sapply(
      conf$languages[sapply(conf$languages, function(l) l$code == c)],
      function(l) l$vectors
    )
  })

  # updating the language status and showing it as pending if we detected work to do
  tasks$languages$status <- (if (
    (!is.na(tasks$languages$status) && tasks$languages$status == "success") &&
      length(to_remove) + length(to_add) + length(to_update) > 0
  )
    "pending" else tasks$languages$status)

  #filtering tasks by status if required and return
  if (length(statuses) == 0) tasks else
  ...
  for (i in sorted_tasks) {
    ...
    if (tasks[[i]]$task %in% c("dependencies", "geonames", "languages")) {
      ...
      if (in_requested_status(tasks[[i]])) {
        ...
        is.na(tasks[[i]]$status) ||
          (# dealing with normal recurrent scheduling. This is the case when status is not set (first time)
          tasks[[i]]$status %in%
            c("pending", "success", "scheduled") && #or if status is pending success or scheduled (ready for next execution)
            {
              ...
              last_ended <-
                Reduce(
                  ...
                  f = function(a, b) {
                    ...
                    } else if (is.na(a$end_on)) b else if (is.na(b$end_on))
                      a else if (a$end_on > b$end_on) a else b
                  }
                )
              one_shot_task_count <- 3 #NUMBER OF ONE SHOT TASKS UPDATE IT ON CHANGE
              next_order <- if (
                last_ended$order + 1 <= max(sapply(tasks, `[[`, "order"))
              )
                last_ended$order + 1 else one_shot_task_count
        ...
        if (is.na(tasks[[i]]$end_on)) {
          # If taks has never ended then schedule it straight away
          tasks[[i]]$scheduled_for <- now + (i - 1) / 1000
        } else if (tasks[[i]]$end_on >= tasks[[i]]$scheduled_for) {
          #if the task has ended after the last schedule (which should always be the case)
          ...
          tasks[[i]]$scheduled_for <- if (
            length(next_slots) > 0 && next_slots[[1]] > now
          )
            next_slots[[1]] else now + (i - 1) / 1000
        }
        change <- TRUE
    ...
    if (
      ...
      i_next <- order(sapply(
        ...
        function(t)
          if (
            !is.na(t$status) &&
              t$status %in% c("aborted", "scheduled", "failed", "running")
          )
            t$order else 999
      ))[[1]]
      ...
      if (tasks[[i_next]]$status != "aborted") {
        ...
        if (tasks[[i_next]]$status %in% c("failed", "running")) {
          tasks[[i_next]]$failures = (if (
            !exists("failures", where = tasks[[i_next]])
          )
            1 else tasks[[i_next]]$failures + 1)
        }
      ...
      last_sleeping_message + (conf$schedule_span * 60) < Sys.time() &&
        length(tasks[sapply(
          tasks,
          function(t)
            !is.na(t$status) &&
      ...
      i_next <- order(sapply(
        tasks,
        function(t)
          if (!is.na(t$status) && t$status %in% c("scheduled")) t$order else 999
      ))[[1]]
      message(paste(
        Sys.time(),
        ": Nothing else todo. Going to sleep each 5 seconds until ",
        format(tasks[[i_next]]$scheduled_for, tz = Sys.timezone(), usetz = TRUE)
      ))
...
in_pending_status <- function(task) {
  ((is.na(task$status) ||
    task$status %in% c("pending", "success", "failure", "aborted")) &&
    ((task$task == "dependencies" &&
      !is.na(conf$dep_updated_on) &&
      (is.na(task$started_on) ||
        task$started_on <
          strptime(conf$dep_updated_on, "%Y-%m-%d %H:%M:%S"))) ||
      (task$task == "geonames" &&
        !is.na(conf$geonames_updated_on) &&
        (is.na(task$started_on) ||
          task$started_on <
            strptime(conf$geonames_updated_on, "%Y-%m-%d %H:%M:%S"))) ||
      (task$task == "languages" &&
        !is.na(conf$lang_updated_on) &&
        (is.na(task$started_on) ||
          task$started_on <
            strptime(conf$lang_updated_on, "%Y-%m-%d %H:%M:%S"))) ||
      (task$task == "alerts" &&
        !is.na(conf$alerts_requested_on) &&
        (is.na(task$started_on) ||
          task$started_on <
            strptime(conf$alerts_requested_on, "%Y-%m-%d %H:%M:%S")))))
}
...
in_requested_status <- function(task) {
  (((task$task == "dependencies" &&
    !is.na(conf$dep_updated_on) &&
    (is.na(task$started_on) ||
      task$started_on < strptime(conf$dep_updated_on, "%Y-%m-%d %H:%M:%S"))) ||
    (task$task == "geonames" &&
      !is.na(conf$geonames_updated_on) &&
      (is.na(task$started_on) ||
        task$started_on <
...
update_languages_task <- function(
  tasks,
  status,
  message,
  start = FALSE,
  ...
  while (i <= length(tasks$languages$code)) {
    if (tasks$languages$codes[[i]] == lang_code && lang_start)
      tasks$languages$statuses[[i]] = "running"
    if (tasks$languages$codes[[i]] == lang_code && lang_done)
      tasks$languages$statuses[[i]] = "done"
  ...
  if (loop_name == "fs") {
    if (
      !is.na(t$dependencies$status) &&
        t$dependencies$status %in% c("success", "running")
    ) {
      ""
