...
generate_alerts <- function(tasks = get_tasks()) {
  tasks <- tryCatch(
    ...
    error = function(error_condition) {
      ...
      message(paste(
        "failed while",
        tasks$alerts$message,
        " :",
        error_condition
      ))
      tasks <- update_alerts_task(
        tasks,
        "failed",
        paste("failed while", tasks$alerts$message, " alerts ", error_condition)
      )
      tasks
    },
...
ears_t_reweighted <- function(
  ...
  step <- (if (same_weekday_baseline) 7 else 1)
  ...
  t(sapply(1:length(ts), function(t) {
    if (t < no_historic * step + 1)
      ...
      compute_threshold <- function(w) {
        ...
        ## Upper threshold
        U0 <- y0bar +
          qt(1 - alpha, df = no_historic - 1) * sd0 * sqrt(1 + 1 / no_historic)
      ...
      w_dw <- ifelse(
        rstar > reweight_threshold,
        (reweight_threshold / rstar)^k_decay,
        1
      )
      ...
      c(
        time_monitored = t,
        y0 = y0,
        U0 = res_noreweight$U0,
        alarm0 = res_noreweight$alarm0,
        y0bar = res_noreweight$y0bar,
        U0_dw = res_dw$U0,
        alarm0_dw = res_dw$alarm0
      )
    }
  })) %>%
    as.data.frame()
...
get_reporting_date_counts <- function(
  ...
  if (nrow(df) > 0) {
    ...
    last_full_hour <- (if (!is.na(end) && end < last_day) 23 else
      as.integer(strftime(
        max(
          as.POSIXlt(df$created_date) + (as.integer(df$created_hour) - 1) * 3600
        ),
        "%H"
      )))
    ...
    df$reporting_date <- ifelse(
      df$created_hour <= last_full_hour,
      df$created_date,
      df$created_date + 1
    )

    # filtering by start date
    df <- (if (is.na(start)) df else
      dplyr::filter(df, .data$reporting_date >= start))
    # filtering by end date
    df <- (if (is.na(end)) df else
      dplyr::filter(df, .data$reporting_date <= end))
    ...
    data.frame(
      reporting_date = as.Date(character()),
      count = numeric(),
      known_users = numeric(),
...
calculate_region_alerts <- function(
  ...
  `%>%` <- magrittr::`%>%`
  read_from_date <- get_alert_count_from(
    date = start,
    baseline_size = no_historic,
  ...
  if (nrow(counts) > 0) {
    ...
    missing_dates <- date_range[sapply(
      date_range,
      function(d) !(d %in% counts$reporting_date)
    )]
    if (length(missing_dates > 0)) {
      missing_dates <- data.frame(reporting_date = missing_dates, count = 0)
      counts <- dplyr::bind_rows(counts, missing_dates) %>%
        dplyr::arrange(.data$reporting_date)
    }
    # Calculating alerts
    alerts <- ears_t_reweighted(
      counts$count,
      alpha = alpha / bonferroni_m,
    ...
    counts <- if (is.na(start)) counts else
...
calculate_regions_alerts <- function(
  ...
  series <- lapply(1:length(regions), function(i) {
    ...
    bonferroni_m <-
      if (bonferroni_correction) {
        length(all_regions[unlist(lapply(
          all_regions,
          function(r) r$level == all_regions[[regions[[i]]]]$level
        ))])
      } else 1
    alerts <-
  ...
  df <- Reduce(x = series, f = function(df1, df2) {
...
get_alert_count_from <- function(date, baseline_size, same_weekday_baseline) {
  if (!same_weekday_baseline) date - (baseline_size + 2) else
...
do_next_alerts <- function(tasks = get_tasks()) {
  ...
  while (is.na(alert_to) || is.na(alert_to_hour)) {
    ...
    if (wait_for > 30)
      stop(
        "Cannot determine the last aggregated period for alert detection. Please check that series have been aggregated"
      )
  }
  ...
  if (!file.exists(alert_file)) {
    ...
    close(f)
    if (
      nrow(existing_alerts) == 0 ||
        nrow(
          existing_alerts %>%
            dplyr::filter(.data$date == alert_to & .data$hour == alert_to_hour)
        ) ==
          0
    )
      do_alerts <- TRUE
...
get_alerts <- function(
  ...
  regions <- get_country_items()
  t <- topic
  cnames <- (if (class(countries) %in% c("numeric", "integer"))
    lapply(countries, function(i) regions[[i]]$name) else countries)
  alert_files <- list.files(
    file.path(conf$data_dir, "alerts"),
    recursive = TRUE,
    full.names = TRUE
  )
  if (length(alert_files) == 0) {
    ...
    file_dates <- lapply(alert_files, function(f) {
      list(
        path = f,
        date = as.Date(
          substr(tail(strsplit(f, "/")[[1]], n = 1), 1, 10),
          format = "%Y.%m.%d"
        )
      )
    })
    files_in_period <- lapply(
      file_dates[sapply(
        file_dates,
        function(fd) fd$date >= as.Date(from) && fd$date <= as.Date(until)
      )],
      function(fd) fd$path
    )
    ...
    df <- Reduce(x = alerts, f = function(df1, df2) {
      dplyr::bind_rows(df1, df2)
    })
    if (!is.null(df)) {
      ...
      df$tops <- paste(
        ...
        ifelse(
          !is.na(df$topwords),
          paste("<li><b>Top Words</b>: ", df$topwords, "</li>\n"),
          ""
        ),
        ifelse(
          !is.na(df$tags),
        ...
        ifelse(
          !is.na(df$entities),
          paste("<li><b>Top Entities</b>: ", df$entities, "</li>\n"),
          ""
        ),
        ifelse(
          !is.na(df$contexts),
          paste("<li><b>Top Contexts</b>: ", df$contexts, "</li>\n"),
          ""
        ),
        ifelse(
          !is.na(df$urls) & nchar(df$urls) > 0,
          paste(
            "<li><b>Top Urls</b>: ",
            sapply(strsplit(df$urls, ", "), function(s) {
              if (!is.null(s) && !all(is.na(s)) && length(s) > 0 && all(nchar(s) > 0)) {
                ...
                counts <- sapply(
                  strsplit(s, " \\("),
                  function(f)
                    paste(" (", if (length(f) > 1) f[[2]] else "-", sep = "")
                )
                paste(
                  sapply(
                    1:length(urls),
      ...
      alert_training <- get_alert_training_df()
      given <- setNames(
        alert_training$given_category,
        tolower(paste(
          alert_training[["date"]],
          alert_training[["topic"]],
          alert_training[["country"]],
          sep = "@@"
        ))
      )
  ...
  df$topposts <- if (nrow(df) == 0) character() else {
    sapply(1:nrow(df), function(i) {
      created_from = (if (df$hour[[i]] == 23) {
        ...
        paste0(as.character(as.Date(df$date[[i]]) - 1), "T0", df$hour[[i]] + 1)
      } else
        paste0(as.character(as.Date(df$date[[i]]) - 1), "T", df$hour[[i]] + 1))
      created_to = (if (df$hour[[i]] == 23) {
        paste0(df$date[[i]], "TZ")
      } else if (df$hour[[i]] < 10) {
        paste0(df$date[[i]], "T0", df$hour[[i]], "Z")
        ...
        posts <- lapply(conf$languages, function(lang) {
          res <- search_posts(
            query = paste0(
              ...
              (if (length(match_codes) == 0 || ignore_place) "" else
                paste0(
                  "text_loc.geo_country_code:",
                  paste0(match_codes, collapse = ";"),
                  " AND "
                )),
              (if (length(topwords[[i]]) == 0) "" else
                paste0(
                  "(",
...
send_alert_emails <- function(tasks = get_tasks()) {
  ...
  if (nrow(subscribers) > 0 && conf$smtp_host != "") {
    for (i in 1:nrow(subscribers)) {
      ...
      topics <- if (is.na(subscribers$Topics[[i]])) NA else
        strsplit(subscribers$Topics[[i]], ";")[[1]]
      dest <- if (is.na(subscribers$Email[[i]])) NA else
        strsplit(subscribers$Email[[i]], ";")[[1]]
      regions <- if (is.na(subscribers$Regions[[i]])) NA else
        strsplit(subscribers$Regions[[i]], ";")[[1]]
      excluded <- if (is.na(subscribers$`Excluded Topics`[[i]])) NA else
        strsplit(subscribers$`Excluded Topics`[[i]], ";")[[1]]
      realtime_topics <- if (is.na(subscribers$`Real time Topics`[[i]])) NA else
        strsplit(subscribers$`Real time Topics`[[i]], ";")[[1]]
      realtime_regions <- if (is.na(subscribers$`Real time Regions`[[i]]))
        NA else strsplit(subscribers$`Real time Regions`[[i]], ";")[[1]]
      alert_categories <- if (
        !"Alert category" %in% colnames(subscribers) ||
          is.na(subscribers$`Alert category`[[i]])
      )
        NA else strsplit(subscribers$`Alert category`[[i]], ";")[[1]]
      slots <- as.integer(
        if (is.na(subscribers$`Alert Slots`[[i]])) NA else {
          times <- strsplit(subscribers$`Alert Slots`[[i]], ";")[[1]]
          times <- sapply(
            times,
            function(t) {
              parts <- as.integer(strsplit(t, "[^0-9]")[[1]])
              if (length(parts) <= 1) parts else parts[[1]] + parts[[2]] / 60
            }
          )
        }
      )
      ...
      user_alerts <- get_alerts(
        topic = if (all(is.na(topics))) NULL else if (
          !all(is.na(realtime_topics))
        )
          unique(c(topics, realtime_topics)) else topics,
        countries = if (all(is.na(regions))) NULL else if (
          !all(is.na(realtime_regions))
        )
          unique(c(regions, realtime_regions)) else regions,
        from = alert_date,
        until = alert_date
      )
      ...
      if (!is.null(user_alerts)) {
        ...
        user_alerts <- (if (!all(is.na(excluded)))
          user_alerts %>% dplyr::filter(!(.data$topic %in% excluded)) else
          user_alerts)

        # Getting last metrics per alert by region and topic
        last_metrics <- list()
        if (nrow(user_alerts) > 0) {
          user_alerts$key = paste(
            user_alerts$topic,
            user_alerts$country,
            sep = "-"
          )
          ...
          user_alerts <- user_alerts %>%
            dplyr::mutate(
              ...
              known_users = unlist(last_metrics[paste(
                .data$key,
                "known_users",
                sep = "-"
              )])
        ...
        current_hour <- as.numeric(strftime(Sys.time(), format = "%H")) +
          current_minutes / 60

        # Getting the current slot
        current_slot <- (if (all(is.na(slots)))
          as.integer(current_hour) else if (
          length(slots[slots <= current_hour]) == 0
        )
          tail(slots, 1) else max(slots[slots <= current_hour]))

        # Evaluating if slot alerts should be sent
        send_slot_alerts <- (all(is.na(slots)) ||
        ...
        instant_alerts <- user_alerts %>%
          dplyr::filter(
            (!all(is.na(realtime_topics)) | !all(is.na(realtime_regions))) &
              ((all(is.na(realtime_topics)) |
                .data$topic %in% realtime_topics) |
                (all(is.na(realtime_regions)) |
                  .data$country %in% realtime_regions))
          )

        # Excluding instant alerts produced before the last alert sent to user
        instant_alerts <- (if (
          exists("date", where = tasks$alerts$sent[[user]]) &&
            exists("hour_instant", where = tasks$alerts$sent[[user]]) &&
            as.Date(tasks$alerts$sent[[user]]$date, format = "%Y-%m-%d") ==
              alert_date
        )
          instant_alerts %>%
            dplyr::filter(
              .data$hour > tasks$alerts$sent[[user]]$hour_instant
            ) else instant_alerts)

        # getting slot alerts
        slot_alerts <- user_alerts %>%
          dplyr::filter(
            ((all(is.na(realtime_topics)) & all(is.na(realtime_regions))) |
              ((!all(is.na(realtime_topics)) &
                !(.data$topic %in% realtime_topics)) &
        ...
        slot_alerts <- (if (
          ...
          slot_alerts %>%
            dplyr::filter(.data$hour > tasks$alerts$sent[[user]]$hour_slot) else
          slot_alerts)
        # Joining back instant and slot alert for email send

        user_alerts <- dplyr::union_all(slot_alerts, instant_alerts) %>%
          dplyr::mutate(topic = get_topics_labels()[.data$topic])
        ...
        if (nrow(user_alerts) > 0) {
          ...
          html <- gsub(
            "@alerts",
            paste(c(non_one_alert_tables, one_alert_table), collapse = "\n"),
            html
          )
          close(t_con)
          ...
          smtp <- (if (
            is.na(conf$smtp_password) ||
              is.null(conf$smtp_password) ||
              conf$smtp_password == ""
          )
            emayili::server(
              host = conf$smtp_host,
              port = conf$smtp_port,
              insecure = conf$smtp_insecure,
              reuse = FALSE
            ) else
            emayili::server(
              host = conf$smtp_host,
              port = conf$smtp_port,
              username = conf$smtp_login,
              insecure = conf$smtp_insecure,
...
get_alert_tables <- function(
  ...
  alert_tables <- (if (nrow(alerts) > 0) {
    tables <- sapply(topics, function(t) {
      paste(
        "<h5>",
        (if (!group_topics) ungrouped_title else t),
        "</h5>",
        alerts %>%
          ...
          print(
            type = "html",
            file = tempfile(),
            sanitize.text.function = function(f) f
          )
      )
    })
    ...
    tables <- gsub(
      "<th ",
      "<th style=\"border: 1px solid black;background-color: #3d6806;color: #ffffff;text-align: center;\" ",
      tables
    )
    tables <- gsub(
      "<th> ",
...
get_alert_training_df <- function() {
  ...
  current <- readxl::read_excel(
    get_alert_training_path(),
  ...
  current %>%
    dplyr::arrange(.data$date, .data$topic, .data$country, .data$topwords)
}

get_alert_balanced_df <- function(
  alert_training_df = get_alert_training_df(),
  progress = function(value, message) {
  }
) {
  allcat_df <- alert_training_df %>%
    dplyr::filter(!is.na(.data$given_category)) %>%
    dplyr::group_by(.data$given_category) %>%
    dplyr::count()
  ...
  split <- jsonlite::rbind_pages(lapply(1:nrow(allcat_df), function(i) {
    ...
    alert_training_df %>%
      ...
      dplyr::mutate(
        test = sample(x, length(x), replace = FALSE) > length(x) * 0.75
      )
  }))

  test_df <- split %>%
    dplyr::filter(.data$test) %>%
    dplyr::mutate(deleted = FALSE, augmented = FALSE)
  ...
  na_cat <- alert_training_df %>%
    dplyr::filter(is.na(.data$given_category)) %>%
    dplyr::mutate(test = FALSE, deleted = FALSE, augmented = FALSE)

  # getting the alerts to augment which are categories with less annotations, we do not consider location for this purpose
  alerts_to_augment <- training_df %>%
    dplyr::filter(.data$given_category %in% cat_to_augment$given_category) %>%
    dplyr::distinct(
      .data$topic,
      .data$date,
      .data$given_category,
  ...
  alerts_to_augment <- add_topposts(
    ...
    1000,
    ignore_place = TRUE,
    progress = function(value, message) progress(value * 0.9, message)
  )
  ...
  if (nrow(cat_to_augment) > 0) {
    ...
    for (icat in 1:nrow(cat_to_augment)) {
      ...
      if (size_after_augment < smallest_cat) smallest_cat <- size_after_augment
    }
  }
    ...
    augmented_alerts <- jsonlite::rbind_pages(list(
      ...
      tibble::tibble(
        ...
        augmented = as.logical(sapply(
          alerts_for_augment,
          function(r) r$augmented
        )),
        test = as.logical(sapply(alerts_for_augment, function(r) r$test))
      )
    ))
  } else augmented_alerts = training_df
  balanced_training <- jsonlite::rbind_pages(lapply(
    1:nrow(cat_df),
    function(i) {
      ...
      augmented_in_cat %>%
        dplyr::mutate(
          deleted = sample(x, length(x), replace = FALSE) > length(x) * frac
        )
    }
  ))
  ...
  balanced_alerts <- jsonlite::rbind_pages(list(
    balanced_training,
    test_df,
    na_cat
  ))
...
get_alert_training_runs_df <- function() {
  ...
  current$`custom_parameters` <- lapply(
...
classify_alerts <- function(
  ...
  `%>%` <- magrittr::`%>%`
  if (is.null(alerts) || nrow(alerts) == 0) alerts else if (
    retrain &&
      min(
        (alerts %>%
          dplyr::group_by(.data$given_category) %>%
          dplyr::summarise(n = dplyr::n()) %>%
          dplyr::filter(!is.na(.data$given_category)))$n
      ) <
        10
  ) {
    warning(
      "In order to train alerts' classifier all categories must have at least 10 observations"
    )
    alerts
  } else {
    runs <- get_alert_training_runs_df()
    augment_classes <- nrow(
      runs %>% dplyr::filter(.data$active & .data$balance_classes)
    ) >
      0
    if (augment_classes && retrain) {
      progress(
        value = 0.2,
        message = "Balancing classes as requested per parameters"
      )
      alerts <- get_alert_balanced_df(
        alerts,
        progress = function(value, message) {
          progress(0.2 + value * 0.5, message)
        }
      )
    } else {
    ...
    body = paste(
      ...
      paste(
        "\"alerts\":",
        jsonlite::toJSON(alerts, pretty = T, auto_unbox = FALSE)
      ),
      if (retrain)
        paste(
          ",\"runs\":",
          jsonlite::toJSON(runs, pretty = T, auto_unbox = TRUE)
        ) else "",
      "}",
      sep = "\n"
    )
    ...
    post_result <- httr::POST(
      url = get_scala_alert_training_url(),
    ...
    if (httr::status_code(post_result) != 200) {
      ...
      res <- jsonlite::fromJSON(httr::content(
        post_result,
        "text",
        encoding = "UTF-8"
      ))
      ...
      if (retrain) {
        ...
        nruns$custom_parameters <- jsonlite::fromJSON(
          jsonlite::toJSON(nruns$custom_parameters),
          simplifyVector = F
        )
...
write_alert_training_db <- function(
  alerts,
  runs = get_alert_training_runs_df()
) {
  `%>%` <- magrittr::`%>%`
  ...
  runs <- runs %>%
    dplyr::transmute(
      ...
      `Active` = .data$active,
      `Documentation` = .data$documentation,
      `Custom Parameters` = lapply(
        .data$custom_parameters,
        function(l) jsonlite::toJSON(l, auto_unbox = TRUE)
      )
    )
  openxlsx::writeDataTable(
    wb,
