...
- `bluesky-plans.R` contains the functions to the Bluesky specific logic to get, update and finish the plans.
  ...
  - `bluesky_new_plan_logic_if_previous_has_passed()` is used to create a new plan if the previous one has passed with the required elements from the previous unfinished plan.
...
The specific logic is implemented in the `new_plan_logic_if_previous_has_passed()` family of functions. For Bluesky it will call `bluesky_new_plan_logic_if_previous_has_passed()`.
