...
trend_line <- function(
  ...
  if (is.character(countries) && length(countries) > 0) {
    ...
    countries = (1:length(reg))[sapply(
      1:length(reg),
      function(i) reg[[i]]$name %in% countries
    )]
  }
...
plot_trendline <- function(
  ...
  fig_line <- ggplot2::ggplot(
    ...
    {
      # Line shadow
      if (length(df$lim_start[!is.na(df$lim_start)]) > 0)
        ggplot2::geom_ribbon(
          ggplot2::aes(
            ymin = .data$lim_start,
            ymax = .data$limit,
    ...
    ggplot2::labs(
      title = ifelse(
        ...
        paste0(
          ...
          date_max,
          "\n in ",
          if (as.integer(countries) == 1) "the world" else
            regions[[as.integer(countries)]]$name,
          " ",
          location_message
        ),
        paste0(
    ...
    ggplot2::scale_x_date(
      ...
      breaks = function(x) {
        ...
        if (days < 15) {
          ...
          strftime(min(x), format = "%d") != strftime(max(x), format = "%d") &&
            years <= 2
        ) {
          ...
          strftime(min(x), format = "%m-%d") !=
            strftime(max(x), format = "%m-%d")
        ) {
...
create_map <- function(
  ...
  if (nrow(df) == 0) {
    return(get_empty_chart(
      "No data found for the selected topic, region and period"
    ))
  }
  ...
  df$Country <- sapply(
    unname(map[df$country_code]),
    function(i) if (!is.na(i)) regions[[i]]$name else NA
  )
  if (!detailed) {
    df$Long <- sapply(
      unname(map[df$country_code]),
      function(i)
        if (!is.na(i)) mean(c(regions[[i]]$minLong, regions[[i]]$maxLong)) else
          NA
    )
    df$Lat <- sapply(
      unname(map[df$country_code]),
      function(i)
        if (!is.na(i)) mean(c(regions[[i]]$minLat, regions[[i]]$maxLat)) else NA
    )
  }
  df$MinLat <- sapply(
    unname(map[df$country_code]),
    function(i) if (!is.na(i)) regions[[i]]$minLat else NA
  )
  df$MaxLat <- sapply(
    unname(map[df$country_code]),
    function(i) if (!is.na(i)) regions[[i]]$maxLat else NA
  )
  df$MinLong <- sapply(
    unname(map[df$country_code]),
    function(i) if (!is.na(i)) regions[[i]]$minLong else NA
  )
  df$MaxLong <- sapply(
    unname(map[df$country_code]),
    function(i) if (!is.na(i)) regions[[i]]$maxLong else NA
  )
  ...
  proj <- (if (!is.null(proj)) proj else if (full_world)
  ...
  countries_geo_wgs84_filtered <- countries_geo_wgs84 %>%
    ...
    dplyr::mutate(
      selected = ifelse(
        .data$iso_a2_clean %in% country_codes,
        "a. Selected",
        "b. Excluded"
      )
    )
  ...
  map_limits <- countries_geo_proj_filtered %>%
    # Reminder: these coordinates are in WGS84
    dplyr::filter(
  ...
  cutsCandidates <- unique(sapply(
    c(maxCount / 50, maxCount / 20, maxCount / 5, maxCount),
    function(v)
      max(1, ceiling((v / (10^floor(log10(v))))) * (10^floor(log10(v))))
  ))
  proj_df$countGroup <- sapply(
    proj_df$count,
    function(c) min(cutsCandidates[c <= cutsCandidates])
  )
  proj_df$plotlycuts <- paste(
    letters[dplyr::dense_rank(proj_df$countGroup) + 3],
    ". ",
    proj_df$countGroup,
    sep = ""
  )
  ...
  countries_geo_proj_filtered$Details <-
    paste(
      "\nRegion:",
      countries_geo_proj_filtered$name,
      sep = ""
    )
...
create_topchart <- function(
  ...
  # plotting
  fig <- (df %>%
    ggplot2::ggplot(ggplot2::aes(x = .data$top, y = .data$frequency)) +
...
get_font_family <- function() {
  if (
    .Platform$OS.type == "windows" &&
      is.null(grDevices::windowsFonts("Helvetica")[[1]])
  )
