...
get_token <- function(request_new = TRUE) {
  ...
  if (exists("access_token", where = conf$twitter_auth)) {
    if (
      new_rtweet &&
        (conf$twitter_auth$access_token != "" || conf$twitter_auth$bearer == "")
    )
      stop(
        "It seems you have updated the rtweet package. You need to reset the twitteer authentication using your app bearer token, fallback to user authentication or downgrade rtweet"
      ) else conf$twitter_auth_mode == "app"
  }
  # An app token is requested
  if (conf$twitter_auth_mode == "app") {
    # if new rtweet then we just provide the registered bearer
    if (new_rtweet)
      token <- conf$twitter_auth$bearer # if not requesting a new one and a non null token is already available on the configuration (because used) then we get it from conf
    else if (
      !request_new && exists("token", where = conf) && !is.null(conf$token)
    )
      token <- conf$token # if not requesting new and an available token is written on disk
    else if (!request_new && file.exists(token_path)) {
      token <- readRDS(token_path)
      if (!("bearer" %in% class(conf$token))) token <- "" # rtweet::bearer_token(token)
    ...
    token <- (if (interactive() && request_new)
      if (new_rtweet) {
        # new version of rtweet detected using new function
        #          t <- rtweet::rtweet_user()
        saveRDS(t, token_path)
        t
      } else '' else if (file.exists(token_path)) { #rtweet::get_token()
      readRDS(token_path)
    } else if (request_new) {
      stop(
        "Cannot get a token on an non interactive session. Create it from configuration page first"
      )
    } else
...
twitter_get <- function(urls, i = 0, retries = 20, tryed = list()) {
  ...
  url <- if (length(urls) > 1) {
    ...
    if (exists("api_ver", last_get))
      toTry <- toTry[order(
        sapply(
          toTry,
          function(v) if (v == last_get$api_ver) paste0("zzz", v) else v
        ),
        decreasing = T
      )]
    toTry <- toTry[[1]]
  ...
  if (is.null(conf$token) || !exists("token", where = conf))
    conf$token <- get_token()
      ...
      httr::GET(
        ...
        if ("bearer" %in% class(conf$token))
          httr::add_headers(
            Authorization = paste(
              "Bearer",
              attr(conf$token, "bearer")$access_token,
              sep = " "
            )
          ) else if (is.character(conf$token))
          httr::add_headers(
            Authorization = paste("Bearer", conf$token, sep = " ")
          ) else httr::config(token = conf$token)
      )
    },
  ...
  if (res$status_code == 200) {
    ...
    if (
      exists("x-rate-limit-remaining", where = res$headers) &&
        as.integer(res$headers[["x-rate-limit-remaining"]]) == 0
    ) {
      ...
      if (length(tryed) < length(url)) {
        ...
        message(paste(
          "The Twitter application endpoint ",
          last_get$api_ver,
          "is about to reach its rate limit, changing endpoint to another available version"
        ))
        ...
        towait <- as.integer(floor(as.numeric(difftime(
          as.POSIXct(
            as.integer(res$headers[["x-rate-limit-reset"]]),
            origin = "1970-01-01"
          ),
          Sys.time(),
          units = "secs"
        ...
        message(paste(
          "The Twitter application endpoint is ",
          last_get$api_ver,
          " about to reach its rate limit, waiting",
          towait,
          "seconds  until",
          Sys.time() + towait,
          " as requested by Twitter"
        ))
        if (towait < 1) {
          towait = i * i
        } else {
        ...
        message(paste(
          "The Twitter application endpoint ",
          last_get$api_ver,
          "is about to reach its rate limit, but no waiting instruction has been detected. Waiting",
          towait,
          "seconds until",
          Sys.time() + towait
        ))
    ...
    message(
      "The Twitter application is slowing down (rate limited) by Twitter, waiting 1 minute before continuing"
    )
    Sys.sleep(60)
    conf$token = NULL
    return(twitter_get(urls, i = i + 1, retries = retries, tryed = tryed))
    ...
    if (length(tryed) < length(url)) {
      ...
      message(paste(
        "The Twitter application endpoint ",
        last_get$api_ver,
        "has reached its rate limit, changing endpoint to another available version"
      ))
      tryed <- c(tryed, last_get$api_ver)
    } else if (exists("x-rate-limit-reset", where = res$headers)) {
      ...
      message(paste(
        "The Twitter application endpoint ",
        last_get$api_ver,
        "has reached its rate limit, waiting",
        towait,
        "seconds  until",
        Sys.time() + towait,
        "as requested by twitter"
      ...
      towait <- 15 * 60
      message(paste(
        "The Twitter application endpoint has reached its rate limit, but no waiting instruction has been detected. Waiting",
        towait,
        "seconds"
      ))
      Sys.sleep(towait + 10)
    ...
    writeLines(
      ...
      paste(conf$data_dir, "lasterror.log", sep = "/")
    )
    return(twitter_get(urls, i = i + 1, retries = retries, tryed = tryed))
    ...
    last_get$api_ver == "2" &&
      {
        ...
        if (
          is.na(
            is.na(mess$errors$parameters) ||
              is.na(names(mess$errors$parameters))
          )
        )
          FALSE else names(mess$errors$parameters) == "since_id"
      }
  ) {
    # Special error in API 2 when query is too old
    message(
      "Error interpreted as too old query on Twitter API v2. Assuming empty result set"
    )
    message(mess)
    stop("too-old")
  } else if (i <= retries) {
    ...
    message(paste(
      "Error status code ",
      res$status_code,
      "returned by Twitter API waiting for",
      i * i,
      "seconds before retry with a new token"
    ))
    ...
    stop(paste(
      "Run out of options to call API after",
      i,
      "retries. URL tryed:",
      url
    ))
