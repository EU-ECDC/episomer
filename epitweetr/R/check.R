...
get_java_version <- function() {
  ...
  if (length(index) == 0) NULL else {
    j_version_parts <- strsplit(
      gsub("[^0-9]", ".", jver[[index[[1]]]]),
      "\\.+"
    )[[1]]
    j_version_parts <- j_version_parts[grepl("[0-9]", j_version_parts)]
    if (length(j_version_parts) > 1 && as.integer(j_version_parts[[1]]) == 1)
      as.integer(j_version_parts[[2]]) else as.integer(j_version_parts[[1]])
  }
}
...
check_winmsvcr100 <- function() {
  ...
  if (.Platform$OS.type == "windows") {
    ...
    if (file.exists(msvcr100Path)) TRUE else {
      warning(paste(
...
check_geonames <- function() {
  if (!file.exists(get_geonames_txt_path())) {
    ...
    ret <- (if (!file.exists(dir)) FALSE else {
      dirs <- list.dirs(dir, recursive = FALSE)
      ind_dir <- dirs[grepl(".*/lucIndex[^/]*", dirs)]
      if (length(ind_dir) == 0) FALSE else
...
check_series_present <- function(
  ...
  rds_counts <- sapply(
    ...
    function(ds)
      length(list.files(
        path = file.path(conf$data_dir, "series"),
        recursive = TRUE,
        pattern = paste(ds, ".*\\.Rds", sep = "")
      ))
  )
  fs_counts <- sapply(
    series,
    function(ds)
      length(list.files(
        path = file.path(conf$data_dir, "fs", ds),
        recursive = TRUE,
        pattern = ".*\\.fdt"
      ))
  )
  if (all(rds_counts > 0) || all(fs_counts > 0)) TRUE else {
    warning(
      "No aggregated series found. Please run the search loop to collect and aggregate data"
    )
    FALSE
  }
}
...
check_alerts_present <- function() {
  dirs <- list.dirs(
    path = file.path(conf$data_dir, "alerts"),
    recursive = FALSE
  )
...
check_scheduler <- function() {
  if (.Platform$OS.type == "windows") {
    installed <- tryCatch(
      {
        func <- taskscheduleR::taskscheduler_create
        TRUE
      },
...
check_manual_task_request <- function() {
  if (
    ...
    warning(
      "Before running the Requirements & alerts pipeline, you have to click on 'activate' Requirements & alerts pipeline button on configuration page"
    )
    FALSE
  } else TRUE
}
      ...
      result = tryCatch(
        {
          processx::run(
            ...
            c(
              "-czf",
              normalizePath(pgz, winslash = "/", mustWork = FALSE),
...
check_all <- function() {
  ...
  data.frame(
    check = names(checks),
    passed = results == "ok",
    message = ifelse(results == "ok", "", results)
  )
}
...
health_check <- function(send_mail = TRUE, one_per_day = TRUE) {
  ...
  if (
    ...
    collection_times <- last_fs_updates(c(
      "topwords",
      "country_counts",
      "geolocated"
    ))
    for (i in 1:length(collection_times)) {
      ...
      if (
        is.na(last_collect) ||
          as.numeric(Sys.time() - last_collect, unit = "hours") >= 1
      ) {
        alerts <- append(
          alerts,
          paste(
            "Serie,",
            serie,
            "has not been produced for more than ",
            as.integer(as.numeric(Sys.time() - last_collect, unit = "hours")),
            "hours"
          )
        )
      }
    ...
    if (
      is.na(last_detect_end) ||
        as.numeric(Sys.time() - last_detect_end, unit = "hours") >=
          (2 + conf$schedule_span / 60)
    ) {
      alerts <- append(
        alerts,
        paste(
          "The alerts tasks has not been finished since",
          as.integer(as.numeric(Sys.time() - last_detect_end, unit = "hours")),
          "hours"
        )
      )
    }
    ...
    for (i in 1:length(tasks)) {
      if (!is.na(tasks[[i]]$status) && tasks[[i]]$status == "aborted") {
        alerts <- append(
          alerts,
          paste("Task,", tasks[[i]]$task, "is on aborted status")
        )
      }
    }
    if (
...
create_snapshot <- function(
  ...
  if ("dependencies" %in% types)
    items <- c(
      ...
      as.list(setNames(
        list.files(get_jars_dest_path(), full.names = TRUE),
        file.path("jars", list.files(get_jars_dest_path()))
      ))
    )
  ...
  to_copy <- file.exists(unlist(items)) &
    (!file.exists(names(items)) |
      names(items) %in% c("detect.log", "fs.log", "search.log"))
  if (length(to_copy) > 0) {
  ...
  cb <- function(line, proc) {
    ...
    if (as.integer(Sys.time()) - last_message > 2) {
      progress(
        1.0 * i / nb,
        paste("Building archive for", i, "of", nb, "files")
      )
      last_message <<- as.integer(Sys.time())
    }
  }
...
epitweetr_files <- function(rel_path, dmin = NULL, dmax = NULL) {
  ...
  for (d in rel_dirs) {
    ...
    file_names <- setdiff(
      list.files(f),
      list.dirs(f, recursive = FALSE, full.names = FALSE)
    )
    files <- as.list(setNames(
      file.path(conf$data_dir, d, file_names),
      file.path(d, file_names)
    ))
    dates <- as.Date(strptime(
      regmatches(
        names(files),
        gregexpr("[0-9][0-9][0-9][0-9]\\.[0-9][0-9]\\.[0-9][0-9]", names(files))
      ),
      "%Y.%m.%d"
    ))
    fd <- ifelse(
      sapply(dates, function(v) is.null(dmin)),
      TRUE,
      ifelse(is.na(dates), FALSE, dates >= dmin)
    ) &
      ifelse(
        sapply(dates, function(v) is.null(dmax)),
        TRUE,
        ifelse(is.na(dates), FALSE, dates <= dmax)
      )
    weeks <- regmatches(
      names(files),
      gregexpr("[0-9][0-9][0-9][0-9]\\.[0-9][0-9]", names(files))
    )
    ...
    fw <- ifelse(
      sapply(weeks, function(v) is.null(wmin)),
      TRUE,
      ifelse(is.na(weeks), FALSE, weeks >= wmin)
    ) &
      ifelse(
        sapply(weeks, function(v) is.null(wmax)),
        TRUE,
        ifelse(is.na(weeks), FALSE, weeks <= wmax)
      )
